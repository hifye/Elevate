    ...
    : IRequestHandler<GetAllEnrollmentByStudentIdQuery, Result<IEnumerable<StudentResponse>>>
{
    ...
    {
        var enrollments = await enrollmentRepository.GetAllEnrollmentByStudentId(query.UserId);
        if (enrollments == null || !enrollments.Any())
            return Result<IEnumerable<StudentResponse>>.Failure("Student not found", "Not Found");
